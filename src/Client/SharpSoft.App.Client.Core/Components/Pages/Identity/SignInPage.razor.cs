...
public partial class SignInPage
{
    ...
    private async Task DoSignInByCode()
    {
        if (isLoading) return;
        ...
        try
        {
            signInModel.ServerAddress = signInModel.ServerAddress!.TrimEnd('/') + "/";
            await AuthenticationManager.SignInByCode(signInModel, CurrentCancellationToken);
            NavigationManager.NavigateTo(RedirectUrl ?? "/");
        }
        catch (KnownException e)
        {
