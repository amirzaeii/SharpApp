...
public partial class AuthenticationManager : AuthenticationStateProvider
{
    ...
    public async Task SignInByCode(SignInRequestDto signInModel, CancellationToken cancellationToken)
    {
        //var result = await httpClient.GetFromJsonAsync<Employee>($"{Configuration.GetApiServerAddress()}api/Hr/EmployeeApi/GetByCode?code={signInModel.Code}");
        var result = await httpClient.GetFromJsonAsync<Employee>($"{signInModel.ServerAddress}api/Hr/EmployeeApi/GetByCode?code={signInModel.Code}");

        if (result is not null)
        {
            await storageService.SetItem("Code", result.Code.ToString(), true);
            await storageService.SetItem("DisplayName", result.DisplayName, true);
            await storageService.SetItem("PersonId", result.Id.ToString(), true);
            await storageService.SetItem("ServerAddress", signInModel.ServerAddress, true);
            ...
            var identity = new ClaimsIdentity(claims:
                [new ("Code", result.Code.ToString()),
                new ("DisplayName", result.DisplayName!) ,
                new ("PersonId", result.Id.ToString()),
            ...
            NotifyAuthenticationStateChanged(Task.Run(() => new AuthenticationState(new ClaimsPrincipal(identity))));

        }
        else
        {
            NotifyAuthenticationStateChanged(Task.Run(() => NotSignedIn()));
        }
    }
